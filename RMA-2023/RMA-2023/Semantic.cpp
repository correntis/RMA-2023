	...
	bool Semantic::semanticsCheck(Lex::LEX& tables, Log::LOG& log)
	{
		...
		for (int i = 0; i < tables.lextable.size; i++)
		{
			...
			case LEX_TYPE:
			{
				...
				{
					sem_ok = false;
					Log::WriteError(log.stream, Error::geterrorin(303, tables.lextable.table[i].sn, tables.lextable.table[i + 1].pos));
				}
				break;
			}
			...
			case LEX_INCR:
			{
				...
				{
					sem_ok = false;
					Log::WriteError(log.stream, Error::geterrorin(314, tables.lextable.table[i].sn, tables.lextable.table[i].endpos));
				}
			...
			case LEX_PROCENT:
			{
					...
					for (k; k > 0; k--)
					{
						if (tables.lextable.table[k].lexema == LEX_ID)
						{
							if (tables.idtable.table[tables.lextable.table[k].idxTI].id == tables.idtable.table[tables.lextable.table[i + 1].idxTI].id)
							{
								...
								{
									sem_ok = false;
									Log::WriteError(log.stream, Error::geterrorin(318, tables.lextable.table[i].sn, tables.lextable.table[k + 2].pos));
								}
							}
						}
					}
				...
				if (tables.lextable.table[i + 1].lexema == LEX_LITERAL)
				{
					...
					{
						sem_ok = false;
						Log::WriteError(log.stream, Error::geterrorin(318, tables.lextable.table[k].sn, tables.lextable.table[i+1].pos));
					}
				}
				break;
			}
					...
					for (int k = i + 1; tables.lextable.table[k].lexema != LEX_SEPARATOR; k++)
					{
						...
						if (tables.lextable.table[k].idxTI != TI_NULLIDX) 
						{
							if (!ignore)
							{
								...
								{
									Log::WriteError(log.stream, Error::geterrorin(314, tables.lextable.table[k].sn, tables.lextable.table[k].pos));
									sem_ok = false;
									break;
								}
							}
						...
						if (lefttype == IT::IDDATATYPE::STR || lefttype == IT::IDDATATYPE::SYM)
						{
							...
							{
								Log::WriteError(log.stream, Error::geterrorin(316, tables.lextable.table[k].sn, tables.lextable.table[i].endpos));
								sem_ok = false;
								break;
							}
						}
						...
						for (int k = i + 1; tables.lextable.table[k].lexema != LEX_BRACELET; k++)
						{
							...
							if (l == LEX_RETURN)
							{
								...
								if (next != TI_NULLIDX)
								{
									...
									{
										Log::WriteError(log.stream, Error::geterrorin(315, tables.lextable.table[k].sn, tables.lextable.table[k + 1].pos));
										sem_ok = false;
										break;
									}
								}
								...
								{
									Log::WriteError(log.stream, Error::geterrorin(315, tables.lextable.table[k].sn, tables.lextable.table[k].endpos));
									sem_ok = false;
									break;
								}
							}
				...
				if (tables.lextable.table[i + 1].lexema == LEX_LEFTHESIS && tables.lextable.table[i - 1].lexema != LEX_FUNCTION) 
				{
					if (e.idtype == IT::IDTYPE::F || e.idtype == IT::IDTYPE::S) 
					{
						...
						for (int j = i + 1; tables.lextable.table[j].lexema != LEX_RIGHTTHESIS; j++)
						{
							...
							if (tables.lextable.table[j].lexema == LEX_ID || tables.lextable.table[j].lexema == LEX_LITERAL)
							{
								...
								if (ctype != e.value.params.types[paramscount - 1])
								{
									...
									Log::WriteError(log.stream, Error::geterrorin(309, tables.lextable.table[i].sn, tables.lextable.table[j].pos));
									sem_ok = false;
									break;
								}
							}
						...
						if (paramscount != e.value.params.count)
						{
							...
							Log::WriteError(log.stream, Error::geterrorin(308, tables.lextable.table[i].sn, tables.lextable.table[i].pos));
							sem_ok = false;
						}
						if (paramscount > 5)
						{
			
							Log::WriteError(log.stream, Error::geterrorin(307, tables.lextable.table[i].sn, tables.lextable.table[i].pos));
							sem_ok = false;
						}
					}
				}
			...
			case LEX_MORE:	case LEX_LESS: case LEX_MORE_E: case LEX_LESS_E:
			{
				...
				if (!flag)
				{
					...
					Log::WriteError(log.stream, Error::geterrorin(317, tables.lextable.table[i].sn, tables.lextable.table[i - 1].pos));
					sem_ok = false;
				}
				break;
			}
